...
def load_pipeline(path="models/mlp_model.pkl"):
    """
    Load a pickled pipeline, fixing cases where custom classes were pickled
    under __main__ or __mp_main__ (common when the model was saved from a script).
    Falls back to dill if available.
    """
    class _FixUnpickler(pickle.Unpickler):
        def find_class(self, module, name):
            # Remap modules that may appear when pickled under __main__ / __mp_main__
            if module in ("__main__", "__mp_main__"):
                module = "moneyballer.mlp_model"
            return super().find_class(module, name)

    with open(path, "rb") as f:
        try:
            return _FixUnpickler(f).load()
        except Exception:
            # Try dill as a fallback if installed (dill can handle many pickles).
            try:
                import dill
                f.seek(0)
                return dill.load(f)
            except Exception as e:
                # Re-raise with context for easier debugging upstream
                raise RuntimeError(f"Failed to load pipeline from {path}: {e}") from e
